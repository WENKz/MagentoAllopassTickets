 ...
 */
class Quentingrisard_CouponAllopass_Model_Type_Onepage {
    ...
    public function saveOrder()
    {
        ...
        mage::log($order->getDiscountAmount(), null, "order.log", true);
        $customer = Mage::getSingleton('customer/session')->getCustomer()->getId();
        $coupon = Mage::getModel("discount/discount")->load($customer, "customer_id");
        $coupon = $coupon->getData();
        $rule = Mage::getModel('salesrule/rule');
        if ($order->getDiscountAmount() < 0)
        {
            $rule->load($coupon["rule_id"]);
            $val = $rule->getDiscountAmount();
            $val = $val - strtr($order->getDiscountAmount(), array("-" => ""));

            mage::log($val, null, "val.log", true);
            $rule->setId($coupon["rule_id"]);
            $rule->setDiscountAmount($val);
